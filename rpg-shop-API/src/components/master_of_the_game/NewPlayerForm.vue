    ...
    methods: {
        createNewPlayer: async function() {
            ...
            let data = JSON.stringify({
                ...
                gold: parseFloat(goldField.value),
                level: parseInt(levelValue),
                strength: parseInt(strengthValue),
                vitality: parseInt(vitalityValue),
                life: parseInt(lifeValue),
                armor: parseInt(armorValue)
            });
            
            let result = null;
            try {
                let response = await fetch('http://rpg.dut-info.cf/rpg/persos/create', {
                    method: 'POST',
                    headers: {
            ...
            let newPlayer = Perso.fromObject(result.data);
