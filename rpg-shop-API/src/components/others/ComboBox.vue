<template>
    <div class="container">
        ...
        <div class="options" :class="(darkTheme === 'true')?'dark':'light'"
            ref="options" >
            <span :key="index" :index="index" v-for="(option, index) in options"
...
export default {
    name: 'ComboBox',
    props: ['hint', 'options', 'darkTheme', 'showSelectedOption', 'getIndex'],
    data() {
        return {
            value: ''
        }
    ...
    methods: {
        ...
        updateSelectedOption: function(event) {
            ...
            this.$refs.options.classList.toggle('show');
            this.$refs.arrow.classList.toggle('rotated');
            if(this.getIndex == true) {
                let index = event.target.getAttribute('index');
                this.$emit('selected-option-changed', index);
            }
            else {
